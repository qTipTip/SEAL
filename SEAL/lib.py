...
def create_cubic_hermite_coefficients(x, f, df):
    ...
    for j in range(k):
        for i in range(m):
            cubic_hermite_coefficients[2 * i, j] = (
                f[i, j] - (1.0 / 3.0) * (x[i + 1] - x[i]) * df[i, j]
            )
            cubic_hermite_coefficients[2 * i + 1, j] = (
                f[i, j] + (1.0 / 3.0) * (x[i + 2] - x[i + 1]) * df[i, j]
            )
...
def approximate_derivatives(x_values, f_values):
    ...
    x_values = np.lib.pad(
        ...
        pad_width=(1, 1),
        mode="constant",
        constant_values=(x_values[2], x_values[m - 3]),
    )
    x_differences = x_values[1:] - x_values[:-1]

    for j in range(k):
        f_component = f_values[:, j]
        f_component = np.lib.pad(
            f_component,
            pad_width=(1, 1),
            mode="constant",
        ...
        for i in range(0, m):
            df_values[i, j] = (
                x_differences[i] * delta_values[i + 1]
                + x_differences[i + 1] * delta_values[i]
...
def parametrize(data_values, data_type="curve", parametrization_type="uniform"):
    ...
    if data_type == "curve":
        ...
        for i in range(1, m):
            parameter_values[i] = parameter_values[i - 1] + np.linalg.norm(
        ...
        raise NotImplementedError(
            "Parametrization of gridded data is not yet implemented"
        )
        m, n, _ = data_values.shape
        parameter_values_x = np.zeros(m)
        parameter_values_y = np.zeros(n)
        for i in range(1, m):
            parameter_values_x[i] = parameter_values_y[i - 1] + np.linalg.norm(
                data_values[i] - data_values[i - 1]
            )
...
def compute_fine_spline_coefficients(p, tau, t, c):
    ...
    if isinstance(c, (list, tuple)) or c.ndim == 1:
        ...
        _, dim = c.shape
    c = np.pad(
        c, pad_width=((p + 1, p + 1), (0, 0)), mode="constant", constant_values=0
    )
...
def evaluate_blossom(p, t, mu, c, x):
    ...
    for i, k in enumerate(range(1, p + 1)):
        ...
        # append 0 to end of first term, and insert 0 to start of second term
        omega = np.divide(
            (x[i] - t1), (t2 - t1), out=np.zeros_like(t1), where=((t2 - t1) != 0)
        )
        b = np.append((1 - omega) * b, 0) + np.insert((omega * b), 0, 0)
