...
from SEAL.lib import (
    create_interpolation_knots,
    create_cubic_hermite_coefficients,
...
def cubic_hermite_interpolation(parameter_values, function_values, derivatives=None):
    cubic_hermite_knots = create_interpolation_knots(
        parameter_values, interpol_type="cubic"
    )
    p = 3
    if derivatives is None:
        derivatives = approximate_derivatives(parameter_values, function_values)

    cubic_hermite_coeff = create_cubic_hermite_coefficients(
        parameter_values, function_values, derivatives
    )
